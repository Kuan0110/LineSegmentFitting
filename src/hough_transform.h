...
class HoughTransform {
 ...
 private:
  ...
  void votePoint(const Point& point, const double delta_range,
                 const double min_range, bool to_add);

  int getLine(LineCoefficients& coeffs, const double min_range,
              const double delta_range,
              const std::vector<std::size_t>& accumulator_cell_indices);

  SegmentClusters seperateDistributedPoints(
      const PointCloudPtr& points, const NeighborIndices& point_indices);

  Point2d getIntersection(const LineSegment2D& line1,
                          const LineSegment2D& line2);

  bool isOverlapping(const LineSegment2D& line_segment,
