...
const std::vector<Vertex> LOOK_UP_TABLE =
    HoughTransform::createCircleLookUpTable();

HoughTransform::HoughTransform(int min_num_vote, int rho_num,
...
bool HoughTransform::run(const PointCloudPtr& cloud,
  ...
  for (size_t i = 0; i < clusters.size(); ++i) {
    ...
    // colorHandler(cluster_cloud, r, g, b); std::cout << "cluster " << i << "
    // size: " << clusters[i].indices.size() << std::endl;

    // // Add the cluster to the viewer
    // viewer.addPointCloud(cluster_cloud, colorHandler, "lane" +
    // std::to_string(i));
...
void HoughTransform::printVote(
  ...
  for (int i = 400; i < 600; ++i) {
    ...
    std::cout << "print line: " << coeffs[0] << "," << coeffs[1] << ","
              << coeffs[2] << std::endl;
    std::cout << "" << std::endl;
  }
}

void HoughTransform::performHT(const PointCloudPtr& cloud,
                               LineSegments& result) {
  ...
  while (remain_points >= min_num_vote_) {
    ...
    LineSegment2D candi_segment(
        cloud, coeffs,
        {Point2d(max_bound.x, max_bound.y), Point2d(min_bound.x, min_bound.y)});
    ...
    if (cluster_to_remove.size() == 0) {
      ...
      for (const auto& point_idx : points_to_remove) {
        ...
        std::cout << "1 closed points: " << cur_point.x << "," << cur_point.y
                  << std::endl;
      }
      if (line_segment.getSegmentLength() > 1) {
        std::cout << "find line endpoint: " << line_segment.endpoints()[0].x()
      ...
      for (int i = 0; i < cluster_to_remove.size(); i++) {
        std::cout << "cur " << i << ", total " << cluster_to_remove.size()
                  << std::endl;
        int num_points = cluster_to_remove[i].size();
        std::cout << "num points: " << num_points << std::endl;
        if (num_points < 8) {
          if (i == cluster_to_remove.size() - 1) removeVote(cur_vote_index_);
        ...
        for (auto index : cluster_to_remove[i]) {
          if (points_to_remove[index].second < 0.5) {
            ...
            // std::cout << "2 closed point: " << cur_point.x << "," <<
            // cur_point.y << std::endl;
          }
        }
        if (count == 0) continue;
        point_matrix.conservativeResize(count, 2);
        // for (int i = 0; i < point_matrix.rows(); ++i) {
        // 	std::cout << "matrix point: " << point_matrix(i,0) << "," <<
        // point_matrix(i,1) << std::endl;
        // }
        LineSegment2D line_segment(candi_segment);
        line_segment.fitLineTLS(point_matrix);

        std::cout << "cluster refined line: " << line_segment.coeffs()[0] << ","
                  << line_segment.coeffs()[1] << "," << line_segment.coeffs()[2]
                  << std::endl;

        if (candi_segment.coeffs()[1] != 0 && line_segment.coeffs()[1] != 0) {
          double angle_candi =
              std::atan2(-candi_segment.coeffs()[0], candi_segment.coeffs()[1]);
          double angle_new =
              std::atan2(-line_segment.coeffs()[0], line_segment.coeffs()[1]);
          std::cout << "angle diff: "
                    << std::abs(angle_candi - angle_new) / PI * 180.0
                    << std::endl;
          ...
          double angle_new =
              std::atan2(-line_segment.coeffs()[0], line_segment.coeffs()[1]);
          std::cout << "angle diff: "
                    << std::abs(PI / 2 - angle_new) / PI * 180.0 << std::endl;
          ...
          double angle_candi =
              std::atan2(-candi_segment.coeffs()[0], candi_segment.coeffs()[1]);
          std::cout << "angle diff: "
                    << std::abs(PI / 2 - angle_candi) / PI * 180.0 << std::endl;
        ...
        for (int j = 0; j < num_points; ++j) {
          ...
          if (new_distance < 0.5) {
            line_segment.clipLineSegment(point);
            // std::cout << "end point: " << curPoint.x << "," << curPoint.y <<
            // std::endl;
          }
          ...
          votePoint(cur_point, delta_range, min_range, false);
          ignore_indices[point_idx.first] = true;
          std::cout << "2 closed points: " << cur_point.x << "," << cur_point.y
                    << std::endl;
          removed_count++;
          // }
        }
...
void HoughTransform::votePoint(const Point& point, const double delta_range,
                               const double min_range, bool to_add) {
  ...
  for (int k = 0; k < theta_num_; k++) {
    ...
    int index =
        std::round((rho_cal - min_range) / delta_range) * theta_num_ + k;
...
int HoughTransform::getLine(
    LineCoefficients& coeffs, const double min_range, const double delta_range,
  ...
  // convert line coefficient from hough space to cartesian space
  cur_vote_index_ = *max_indices;
  const int num_vote = accumulator_[cur_vote_index_];
  ...
  std::cout << "candidate line: " << coeffs[0] << "," << coeffs[1] << ","
            << coeffs[2] << ",num vote: " << num_vote << std::endl;
  std::cout << "" << std::endl;
...
SegmentClusters HoughTransform::seperateDistributedPoints(
  ...
  std::cout << "line segment cluster number: " << point_clusters.size()
            << std::endl;
...
void HoughTransform::intersectLineSegments(LineSegments& line_segments) {
  ...
  for (int i = 0; i < closed_pairs.size(); ++i) {
    Point2d cross_point =
        getIntersection(line_segments[closed_pairs[i].first / 2],
                        line_segments[closed_pairs[i].second / 2]);

    Point2d p1 = line_segments[closed_pairs[i].first / 2]
                     .endpoints()[closed_pairs[i].first % 2];
    Point2d p2 = line_segments[closed_pairs[i].second / 2]
                     .endpoints()[closed_pairs[i].second % 2];
    double d1 = calcDist(cross_point, p1);
    double d2 = calcDist(cross_point, p2);

    if (d1 < 2.0 && d2 < 2.0) {
      if (distances[i] < 1.0) {
        line_segments[closed_pairs[i].first / 2].setEndpoints(
            closed_pairs[i].first % 2, cross_point);
        line_segments[closed_pairs[i].second / 2].setEndpoints(
            closed_pairs[i].second % 2, cross_point);
      } else {
      ...
      line_segments[closed_pairs[i].first / 2].setEndpoints(
          closed_pairs[i].first % 2, mid);
      line_segments[closed_pairs[i].second / 2].setEndpoints(
          closed_pairs[i].second % 2, mid);
    }
  }
...
Point2d HoughTransform::getIntersection(const LineSegment2D& line1,
                                        const LineSegment2D& line2) {
  if (line1.coeffs()[1] == 0) {
